...
def _get_best_model(timeseries):
    ...
    for p in pq_range:
        for d in d_range:
            for q in pq_range:
                try:
                    tmp_mdl=ARIMA(timeseries, order=(p,d,q)).fit(method='mle', trend='nc')
                    tmp_aic=tmp_mdl.aic
                    if tmp_aic < best_aic:
                        best_aic = tmp_aic
                        best_order=(p, d, q)
...
def _check_heteroskedastic_behavior(residuals):
    ...
    lbvalue, pvalue_lb = acorr_ljungbox(residuals**2, lags=[20], boxpierce=False)
...
def _get_garch_model(residuals):
    ...
    for p in pq_range:
        for o in o_range:
            for q in pq_range:
                try:
                    tmp_model = arch_model(residuals, p=p, o=o, q=q, dist='StudentsT').fit(update_freq=5, disp='off')
...
if __name__=="__main__":
    ...
    lrets=np.log(data/data.shift(1)).replace([np.inf, -np.inf], np.nan).dropna()
